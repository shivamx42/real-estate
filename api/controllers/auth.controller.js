...
export const google = async (req, res, next) => {
  try {
    ...
    if (user) {
      ...
      const generatedPassword =   // we have to create a password as password is required to be stored in the database
        Math.random().toString(36).slice(-8) +
        Math.random().toString(36).slice(-8);
      const hashedPassword = bcryptjs.hashSync(generatedPassword, 10);
      const newUser = new User({
        username:   // username is changed to be made as connected
